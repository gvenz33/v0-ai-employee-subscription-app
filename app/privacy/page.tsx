...
export async function generateMetadata(): Promise<Metadata> {
  ...
  if (brand) {
    return {
      title: `Privacy · ${brand}`,
      description: ctx?.meta_description || "Privacy information",
    }
  }
  return {
...
export default async function PrivacyPage() {
  ...
  const host = normalizeHost(h.get("x-forwarded-host") || h.get("host") || "")
  const ctx = await getBrandedPublicContextForHost(host)
  const ext = ctx?.privacy_policy_url?.trim()
      ...
      <main className="mx-auto max-w-4xl px-6 py-16">
        {ctx?.brand_name && (
          <p className="mb-6 rounded-md border border-border bg-muted/50 px-4 py-3 text-sm text-muted-foreground">
            This page describes the 247 AI Employees platform. {ctx.brand_name} may provide additional policies to you
            separately.
          </p>
        )}
